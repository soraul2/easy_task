...
enum PlannerWidgetMonthSelectionStore {
    private static let selectedMonthDayKey = "plannerWidget.selectedMonthDayKey"
...
struct ChangePlannerWidgetMonthIntent: AppIntent {
    static let title: LocalizedStringResource = "플래너 위젯 월 이동"
    static let description = IntentDescription("PlanBase 플래너 위젯의 표시 월을 이동합니다.")
    ...
    func perform() async throws -> some IntentResult {
        PlannerWidgetMonthSelectionStore.move(by: monthDelta)
        WidgetCenter.shared.reloadTimelines(ofKind: CalendarWidgetConstants.plannerKind)
        return .result()
    }
}

struct ResetPlannerWidgetMonthIntent: AppIntent {
    static let title: LocalizedStringResource = "플래너 이번 달 보기"
    static let description = IntentDescription("PlanBase 플래너 위젯을 이번 달로 되돌립니다.")
    static let isDiscoverable = false
    static let openAppWhenRun = false

    func perform() async throws -> some IntentResult {
        PlannerWidgetMonthSelectionStore.reset()
        WidgetCenter.shared.reloadTimelines(ofKind: CalendarWidgetConstants.plannerKind)
        return .result()
    }
}
