...
- 레거시 이미지가 회고당 10개를 넘으면 초과분은 이관을 보류하고 원본 참조를 유지한다.
...
- macOS와 iOS 회고 작성·기록 화면은 첨부와 남은 기존 파일을 중복 제거해 함께 표시하고 부분 이관 상태에서는 이미지 편집을 잠근다.
...
- 반복 V1/V2 가져오기, 무결성 정리 후 재가져오기와 동일 시각 후보의 순서 독립 수렴 통과
...
2. Phase 6에서 보드·캘린더·기록 조회 범위를 제한하고 이미지 디코딩 캐시를 추가한다.
3. 10,000개 작업 기준 성능 검증이 끝난 뒤 CloudKit 개발 컨테이너를 연결한다.
4. 실기기 두 대에서 충돌·이미지·재설치·반복 백업 병합을 통과하기 전에는 운영 스키마를 승격하지 않는다.
