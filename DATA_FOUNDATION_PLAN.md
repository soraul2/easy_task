...
- 복원은 전체 패키지를 먼저 검증하고 `(id, instanceID)` 후보를 병합한 뒤 무결성 정리와 최종 저장을 한 번 수행한다.
- JSON V1은 결정적 인스턴스 ID를 부여해 비파괴 병합하며 포함되지 않은 이미지 원본 목록을 보고한다.
- macOS와 iOS 회고 작성·기록 화면은 첨부 데이터를 기준으로 표시하고 기존 파일은 읽기 전용 fallback으로만 사용한다.
...
- 반복 V1/V2 가져오기와 동일 시각 후보의 순서 독립 수렴 통과
- iOS/macOS Debug/Release 빌드 통과
