...
1. `feature/media-backup-v2`에서 회고 첨부 모델과 이미지 저장 계약을 먼저 확정한다.
2. 기존 파일명/DiaryBlock 이미지를 새 첨부 모델로 옮기는 파일 기반 마이그레이션을 검증한다.
3. 첨부 계약이 고정된 뒤 백업 패키지, 체크섬, merge/upsert 복원을 구현한다.
4. 이미지와 백업 V2가 통과한 뒤 조회 성능 개선과 CloudKit 개발 환경 연결로 이동한다.
