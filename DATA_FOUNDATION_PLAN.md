...
- 데스크톱 기록 목록은 `LazyVStack`을 사용한다.
- 회고별 첨부와 레거시 이미지 블록을 한 번 인덱싱해 카드 단위 반복 전체 검색을 줄였다.

남은 작업:

- Board/Calendar/Archive/Review의 전체 `@Query`를 조건 fetch와 페이지 조회로 전환한다.
- 기록 검색 debounce와 10,000개 작업 성능 기준을 자동화한다.
