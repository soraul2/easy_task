...
현재 일반 실시간 카드는 오늘 todo 또는 doing이 남아 있으면 생성·유지하고, 둘 다 없으면 종료한다.
변경 버튼은 표시할 작업만 바꾸며 todo에는 시작, doing에는 완료를 제공한다. 유효한 선택은 기기에
보존하고 같은 Activity를 갱신한다. 이전 1초 공통 gate는 선택 토큰·최신 상태 검증으로 대체했다.
Focus 우선순위와 미래 알림 완료 확인, 기존 정적 위젯의 시작 정책은 유지한다.

현재 계약과 검증은 [남은 작업 카드 후속 계획](LOCK_SCREEN_TASK_SELECTION_FOLLOWUP.md) 및
[구현 결과](CARRYOVER_AND_REVIEW_DISCOVERY_RESULTS.md)를 따른다. 아래 build 43~60의 구현·출시 기록과
원래 단계별 체크리스트에 있는 ‘doing만 유지/화살표가 상태 변경’은 당시 동작이며 현재 계약이 아니다.
