...
- 시스템 알림 식별자는 `planbase.task-reminder.<task.id>`로 고정한다.
  이전 빌드의 `easytask.task-reminder.<task.id>`도 정리 대상으로 인식한다.
...
- `AppModels`와 `PlanBaseContainerFactory`의 현재 스키마를 V4로 변경한다.
- 기존 Task는 마이그레이션 후 `reminderAt == nil`이어야 한다.

### 무결성 규칙

- 유한하지 않은 알림 시각은 `nil`로 정리한다.
- 완료 여부와 무관하게 유효한 `reminderAt`은 분 단위로 정규화해 보존한다.
- 완료 Task의 값은 예약 원본이 아니라 사용자가 설정했던 알림 기록으로 취급한다.
...
- JSON V1과 패키지 V2 가져오기는 필드 부재와 명시적 `nil`을 구분할 수 없으므로
  기존 로컬 알림을 보존한다. 명시적인 알림 해제는 패키지 V3에서만 적용한다.
...
`PlanBaseCore`에는 UserNotifications에 의존하지 않는 순수 규칙만 둔다.
...
`PlanBaseMobile`에 `TaskNotificationScheduler`를 둔다.
...
- 변경 시 같은 식별자의 요청을 교체
- 완료 시 `reminderAt` 기록은 보존하고 pending/delivered 알림만 제거
- 삭제 또는 사용자의 명시적 알림 해제 시 pending/delivered 알림 제거
...
3. `planbase.task-reminder.`와 레거시 `easytask.task-reminder.` prefix의 pending 요청을 조회한다.
...
- 미완료 미래 알림 카드에는 `bell` 아이콘과 짧은 시각 표시
- 미완료 과거 알림은 `지난 알림`, 완료 Task의 보존값은 `설정했던 알림`으로 표시한다.
- 완료 Task에는 알림 편집을 비활성화하되 보존된 시각은 계속 보여준다.
...
- 알림 변경과 해제 확인
- 완료 직후 시스템 알림은 취소되고 `reminderAt` 기록은 유지되는지 확인
- 삭제 직후 시스템 알림이 취소되는지 확인
...
- 완료 Task의 시스템 예약은 남지 않지만 `reminderAt` 기록은 유지된다.
- 삭제된 Task의 시스템 알림은 남지 않는다.
