...
스크립트는 2099-12-31의 숨은 진단 이벤트에 매번 새로운 UUID를 사용한다. macOS 생성 →
iPhone 확인 → macOS 삭제 → iPhone 소멸 확인과 그 반대 방향을 모두 실행하고, 끝난 뒤
진단 레코드를 삭제한다. 이 프로브는 컨테이너·계정·서명·기본 create/delete 전파만
검증하며 같은 레코드의 오프라인 충돌과 이미지 원본 복구를 대신하지 않는다.
