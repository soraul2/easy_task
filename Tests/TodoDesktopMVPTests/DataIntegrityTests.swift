...
func duplicateReviewUsesInstanceIDToBreakEqualTimestampTie() throws {
    ...
    #expect(activeReview.id == canonicalID)
    #expect(activeReview.instanceID == testUUID(1))
    #expect(activeReview.content == "higher instance")
    #expect(activeReview.createdAt == createdAt)
    #expect(winner.supersededAt == updatedAt)
    #expect(block.reviewId == canonicalID)
...
func seededTemplateMergeRewiresItemsAndPlacements() throws {
    ...
    #expect(canonicalTemplate.supersededAt == nil)
    #expect(canonicalTemplate.name == "Current routine")
    #expect(canonicalTemplate.isFavorite)
    #expect(winnerTemplate.supersededAt == winnerTemplate.updatedAt)
    #expect(canonicalItem.supersededAt == nil)
    #expect(canonicalItem.templateId == canonicalTemplateID)
    #expect(canonicalItem.title == "Current item")
    #expect(winnerItem.supersededAt == winnerItem.updatedAt)
    #expect(placement.sourceTemplateId == canonicalTemplateID)
...
func duplicateLogicalIDsMergeForEveryPersistedModel() throws {
    ...
    #expect(eventA.title == "New event")
    #expect(templateA.name == "New template")
    #expect(itemA.title == "New item")
    #expect(placementA.templateName == "New placement")
    #expect(taskA.title == "New task")
    #expect(reviewA.content == "New review")
    #expect(blockA.text == "New block")
    #expect(eventA.createdAt == oldTimestamp)
    #expect(eventB.supersededAt == newTimestamp)
}
